...
# 1. add_part! actually calls add_parts! with a single argument, so add_parts! is primitive
# 2. rem_parts! calls rem_part! repeatedly, so rem_part! is primitive
# 3. set_suparts! calls set_subpart!, so set_subpart! is primitive
# 4. only have clear_subpart!
