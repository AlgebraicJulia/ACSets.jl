...
@acset_type MyDataInjIdxBit(MySch, unique_index=[:f], part_type=BitSetParts)

@acset_type MyDataNoIdxMark(MySch, part_type=MarkAsDeleted)
@acset_type MyDataIdxMark(MySch, index=[:f], part_type=MarkAsDeleted)
@acset_type MyDataInjIdxMark(MySch, unique_index=[:f], part_type=MarkAsDeleted)

data = @acset MyDataNoIdxBit begin
    A = 5
    B = 5
    f = [1,2,3,4,5]
end
